...
class SessionGraph(Module):
    def __init__(self, opt, n_node):
        ...
        self.LatEx = LatEx(self.hidden_size, self.threshold, self.batch_size, self.slack, self.temperature)
    ...
    def compute_scores(self, hidden, mask, target, item_to_cat, cat_to_item):

        # ----------- intent learning (Explicit)------- #
        if self.intent_learning == 'average':
            cat_to_item = {k: sorted(v) for k, v in sorted(cat_to_item.items())}
            x = cat_to_item.values()
            y = max(map(len, cat_to_item.values()))
            new = []
            count = []
            for i in x:
                q1 = list(i) + [0] * (y - len(i))
        ...
        LatEx_loss = self.LatEx(a, self.item_2cluster, self.item_centroids, target)
        return scores, LatEx_loss * self.scale
...
def forward(model, i, data, item_to_cat, cat_to_item):
    ...
    loss, LatEx_loss = model.compute_scores(seq_hidden, mask, targets, item_to_cat, cat_to_item)
    return targets, loss, LatEx_loss

def train_test(model, opt, train_data, test_data, item_to_cat, cat_to_item, n_node):
    ...
    for i, j in tqdm(zip(slices, np.arange(len(slices))), total=len(slices)):
        if opt.intent_learning == 'cluster' and j % 5 == 0:
            model.e_step()
        model.optimizer.zero_grad()
        targets, scores, LatEx_loss = forward(model, i, train_data, item_to_cat, cat_to_item)
        targets = trans_to_cuda(torch.Tensor(targets).long())
        loss = model.loss_function(scores, targets - 1) + LatEx_loss
        loss.backward()
        model.optimizer.step()
        total_loss += loss.item()
        if j % int(len(slices) / 5 + 1) == 0:
            print('[%d/%d] Loss: %.4f' % (j, len(slices), loss.item()))
            print('[%d/%d] LatEx_loss: %.4f' % (j, len(slices), LatEx_loss))
    ...
    for i in slices:
        targets, scores, ccl_loss = forward(model, i, test_data, item_to_cat, cat_to_item)
